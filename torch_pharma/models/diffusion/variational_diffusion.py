...
from torch_pharma.features import centralize
from torch_pharma.models.diffusion.noise import GammaNetwork, PredefinedNoiseSchedule
from torch_pharma.models.transformers import NumNodesDistribution, inflate_batch_array
from torch_pharma.utils.io import num_nodes_to_batch_index

patch_typeguard()

H_INPUT_TYPE = torch.Tensor

def make_and_save_network_graphviz_plot(*args, **kwargs):
    # TODO: Implement or import if available
    pass
...
class EquivariantVariationalDiffusion(nn.Module):
    ...
    def __init__(
        ...
        loss_type: str = "l2",
        norm_values: List[float] = [1.0, 4.0, 10.0],
        norm_biases: List[Optional[float]] = [None, 0.0, 0.0],
        ...
        assert parametrization in ["eps"], "Epsilon is currently the only supported parametrization."
        assert loss_type in ["vlb", "l2"], "Variational lower-bound and L2 losses are currently the only supported diffusion loss functions."
        assert noise_schedule in ["cosine", "polynomial_n", "polynomial_2", "learned"], "noise_schedule must be one of cosine, polynomial_n, polynomial_2, or learned"
        
        if noise_schedule == "learned":
            assert loss_type == "vlb", "A noise schedule can only be learned with a variational lower-bound objective."
        ...
        self.num_node_scalar_features = num_atom_types + include_charges
        ...
        if noise_schedule == "learned":
            ...
            self.gamma = PredefinedNoiseSchedule(
                 noise_schedule, num_timesteps, noise_precision,
            )
    ...
    def log_pxh_given_z0_without_constants(
        ...
        h_integer = torch.round(
            h_integer * self.norm_values[2] + self.norm_biases[2]
        ).long()
    ...
    def normalize(
        ...
        if generate_x_only:
            bias = self.norm_biases[1] if self.norm_biases[1] is not None else 0.0
            h = (h.float() - bias) / self.norm_values[1]
            return x, h

        # cast to float in case `h` still has `long` or `int` type
        bias_cat = self.norm_biases[1] if self.norm_biases[1] is not None else 0.0
        h_cat = (h["categorical"].float() - bias_cat) / self.norm_values[1]
        h_cat = h_cat * node_mask.float().unsqueeze(-1)
        
        bias_int = self.norm_biases[2] if self.norm_biases[2] is not None else 0.0
        h_int = (
            (h["integer"].float() - bias_int) / self.norm_values[2]
        )
    ...
    def unnormalize(
        ...
        bias_cat = self.norm_biases[1] if self.norm_biases[1] is not None else 0.0
        h_cat = h_cat * self.norm_values[1] + bias_cat
        h_cat = h_cat * node_mask.float().unsqueeze(-1)
        
        bias_int = self.norm_biases[2] if self.norm_biases[2] is not None else 0.0
        h_int = h_int * self.norm_values[2] + bias_int
    ...
    def delta_log_px(self, num_nodes: TensorType["batch_size"]) -> TensorType["batch_size"]:
        ...
        return -d * np.log(self.norm_values[0])
    ...
    def atom_types_and_coords_forward(
        ...
        t_is_not_zero = torch.ones_like(t_is_zero) - t_is_zero
        ...
        self_cond = None
        self_conditioning = (
            self.training and self.self_condition and not (t_int == self.T).any() and random() < self_conditioning_prob
        )
    ...
    def mol_gen_sample(
        ...
        x, h = self.sample_p_xh_given_z0(
            ...
            context=context,
            fix_noise=fix_self_conditioning_noise if self.self_condition else fix_noise,
            generate_x_only=generate_x_only,
            xh_self_cond=self_cond
        )
