...
from torch_pharma.features.tokenizer.tokenize_3d import TOKENIZER
from torch_pharma.constants.periodic_table import periodic_table
from torch_pharma.constants.amino_acids import aas
from torch_pharma.constants.geometry import AA_GEOMETRY
from torch_pharma.constants.atom import *
from torch_pharma.constants.bond_parameters import *
from torch_pharma.molecules.hierarchy import *
from torch_pharma.molecules.utils.bond_type import bond_type_from_rdkit
...
class MoleculeVocab:
    ...
    def __init__(self):
        ...
        self.idx2block = [self.block_dummy] + aas + frags
        ...
        for i, (symbol, abrv) in enumerate(self.idx2block):
            ...
            self.aa_mask.append(True if abrv in AA_GEOMETRY else False)

        # atom level vocab
        self.atom_dummy = 'dummy'
        self.idx2atom = [self.atom_dummy] + periodic_table
        self.atom2idx = {}
        ...
        for _, aa in aas:
            for atom in AA_GEOMETRY[aa]['atoms'] + backbone_atoms:
                atom_comp = AA_GEOMETRY[aa]['types'][atom]
        ...
        self.chemical_bonds = {}
        for symbol, _ in aas:
            self.atom_canonical_orders[symbol] = backbone_atoms + sidechain_atoms[symbol]
            self.element_canonical_orders[symbol] = [name[0] for name in backbone_atoms + sidechain_atoms[symbol]] # only C, N, O, S
            atom2order = { a: i for i, a in enumerate(self.atom_canonical_orders[symbol]) }
            
            self.chemical_bonds[symbol] = [
                (atom2order[bond[0]], atom2order[bond[1]], BondType(bond[2])) for bond in aa_bonds[symbol]
            ]
    ...
    def ca_channel_idx(self):
        return backbone_atoms.index('CA')
