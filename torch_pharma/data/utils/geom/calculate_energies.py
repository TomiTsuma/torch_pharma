...
from torch_pharma.data.xtb_energy import calculate_xtb_energy
...
def process(dataset, split):
    if dataset == "drugs":
        from torch_pharma.data.geom.geom_dataset_adaptive import (
            GeomDrugsDataset as DataModule,
        )

        root_path = "~/.torch_pharma/data/geom"
    elif dataset == "qm9":
        from torch_pharma.data.qm9.qm9_dataset import GeomDrugsDataset as DataModule

        root_path = "~/.torch_pharma/data/qm9"
    else:
