...
QM9_DATA_URL = "https://drive.google.com/uc?id=1uu_bQv34dfoFLR0J8-24OLIj0NCPoXKI"
QM9_UNCHARACTERIZED_URL = "https://drive.google.com/uc?id=1DDBh3PCxZlRVh4odqaYOsErlVtiyr6Rj"
...
def process_xyz_files(data, process_file_fn, file_ext=None, file_idx_list=None, stack=True):
    ...
    if stack:
        molecules = {key: pad_sequence(val, batch_first=True) if val[0].dim(
        ) > 0 else torch.stack(val) for key, val in molecules.items()}
...
def process_xyz_gdb9(datafile):
    ...
    prop_strings = ["tag", "index", "A", "B", "C", "mu", "alpha",
