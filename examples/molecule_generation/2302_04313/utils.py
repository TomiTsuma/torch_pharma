...
from torch_pharma.data.components.edm.edm_dataset import ProcessedDataset
...
async def initialize_datasets(args, dataset, subset=None, splits=None,
    ...
    """
    qm9Dataset = QM9Dataset(calculate_thermo=True)
    ...
    await qm9Dataset.download()
    qm9Dataset.process()
    datafiles = {split: os.path.join(
        *(qm9Dataset.gdb9_dir + [split + ".npz"])) for split in split_names}
...
def prepare_context(
    ...
    batch_size, num_nodes = (batch.num_graphs, batch[positions_key].shape[0])
